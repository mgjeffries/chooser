...
export const ChoiceProvider = (props) => {
  ...
  const getChoices = () => {
    return (
      fetch("http://chooser-server.herokuapp.com/choices")
        .then((res) => res.json())
  ...
  const addChoice = (choice) => {
    let addedchoice = {};
    return fetch("http://chooser-server.herokuapp.com/choices", {
      method: "POST",
      headers: {
  ...
  const editChoice = (choice) => {
    return fetch(`http://chooser-server.herokuapp.com/choices/${choice.id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
